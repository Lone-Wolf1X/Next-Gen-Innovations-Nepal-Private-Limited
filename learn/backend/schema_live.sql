...
CREATE TABLE IF NOT EXISTS `questions` (
  ...
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (`set_id`) REFERENCES `mcq_sets`(`id`) ON DELETE CASCADE
);
...
INSERT INTO `exams` (`category_id`, `title`, `emoji_icon`, `color_hex`) VALUES 
(1, 'Kharidar', '📝', '#173B7A'),
(1, 'Nayab Subba', '🇳🇵', '#F59E0B'),
(2, 'Rastriya Banijya Bank', '🏦', '#16A34A');

INSERT INTO `mcq_sets` (`exam_id`, `title`, `set_date`, `status`) VALUES 
(1, 'Constitution Basics', CURDATE(), 'PUBLISHED'),
(1, 'Daily Mission - Set 2', CURDATE(), 'UNDER_REVIEW');

INSERT INTO `questions` (`set_id`, `question_text`, `option_a`, `option_b`, `option_c`, `option_d`, `correct_option`, `explanation`, `exam_tip`, `is_verified`) VALUES 
