...
def add_ctet_data():
    ...
    # 1. Ensure CTET category exists
    cursor.execute("SELECT id FROM exam_categories WHERE name = 'CTET'")
    cat = cursor.fetchone()
    if cat:
        category_id = cat['id']
    else:
        category_id = str(uuid.uuid4())
        cursor.execute("INSERT INTO exam_categories (id, name, description, icon) VALUES (%s, %s, %s, %s)", 
                       (category_id, "CTET", "Central Teacher Eligibility Test", "📚"))
    
    # 2. Ensure CDP subject exists under CTET
    cursor.execute("SELECT id FROM subjects WHERE name = 'Child Development and Pedagogy' AND category_id = %s", (category_id,))
    subj = cursor.fetchone()
    if subj:
        subject_id = subj['id']
    else:
        subject_id = str(uuid.uuid4())
        cursor.execute("INSERT INTO subjects (id, category_id, name) VALUES (%s, %s, %s)", 
                       (subject_id, category_id, "Child Development and Pedagogy"))
    ...
    for item in ctet_data:
        ...
        """, (
            ...
            item['explanation'],
            1.00,
            0.00 # Usually CTET has no negative marking, but default is 0.25. Let's make it 0.
        ))
    ...
    """, (
        modelset_id,
        "CTET CDP 25-Question Mock Set",
        "A comprehensive 25-questions mock model MCQ set covering CTET CDP syllabus.",
        category_id,
        json.dumps(question_ids),
        30, # 25 questions -> 30 mins
        25.00,
        "published"
    ))
    ...
    print(f"Successfully added CTET category, CDP subject, 25 questions, and Model Set ({modelset_id}).")
